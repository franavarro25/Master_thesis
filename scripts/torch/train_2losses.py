...
from utils import run_epoch, divide_volume
...
parser.add_argument('--loss-feat', default='cx_patches',type=str,
...
if args.wandb:
    ...
    run = wandb.init(project='voxelmorph',group= 'our_loss_patchess',
                     name= args.loss_rgb+'_'+args.loss_feat+'_lambda='+str(args.weight),
...
model, model_dir = vxm_model(model_n = args.loss_rgb+'_'+args.loss_feat+'/our_loss_patches/'+'lambda_'+str(args.weight), 
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    if epoch % 20 == 0:
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                #print('flow_field shape', flow.shape)
                ...
                loss1 = losses[1](y_source, target) * weights[0]
                loss_list.append(loss1.item())
                loss2 = losses[2](flow, torch.zeros((flow.shape))) * weights[2]
        ...
        if  args.wandb:
    	    wandb.log({'dice_score(val)':dice_score.item(),
                       ...
                       'feat_loss':epoch_val_loss[1],
                       'reg_loss':epoch_val_loss[2]}, step=s)
        ...
        print('epoch_val_losses', epoch_val_loss[0], epoch_val_loss[1], epoch_val_loss[2])
    ...
    for data in train_loader:
        #with torch.autograd.set_detect_anomaly(True):
            ...
            #print('data',d,'parameters',params[2])
            ...
            #print('target shape to divide', target.shape)
            #print('y_source shape to divide', y_source.shape)
            #print('flow shape to divide', flow.shape)
    ...
    if  args.wandb:
    	wandb.log({'epoch':epoch, 'time(sec/step)':np.mean(epoch_step_time), 'loss':(np.mean(epoch_total_loss)).item(),
                    ...
                    'loss_feat':np.mean(epoch_loss,axis=0)[1],
                    'reg_loss':np.mean(epoch_loss,axis=0)[2]}, step=s)
    print(' - '.join((epoch_info, time_info, loss_info)), flush=True)
