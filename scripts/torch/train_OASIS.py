...
parser.add_argument('--loss-feat', default='cobi_contra_patches_local',type=str,
...
if args.wandb:
    ...
    run = wandb.init(project='voxelmorph',group= 'CX_contra',
                     name= 'y_norm_bw0.1_'+args.loss_feat+'_lambda='+str(args.weight),
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    if epoch % 20 == 0 or (epoch+1)==args.epochs:
        if epoch % 20 ==0: model.save(os.path.join(model_dir, '%04d.pt' % epoch))
        model.eval()
        dice_score = []
        hausdorff_distance = []
        SDlog_Jac = []
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                #print('target values shape,min,max,mean,std', target.shape,target.min(),target.max(),target.mean(),target.std())
    ...
    for data in train_loader:
        #with torch.autograd.set_detect_anomaly(True):
            ...
            loss_list.append(loss1.item())
            loss = loss0 + loss1
            #print('lass losses 0,1,main', loss0.device, loss1.device, loss.device)
            ...
            loss.backward()
            optimizer.step()
            del  source, target, y_source, flow, loss0, loss1, loss
