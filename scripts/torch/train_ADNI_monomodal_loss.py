...
from utils import run_epoch_multimodal_t11loss, divide_volume
...
parser.add_argument('--loss-feat', default='cobi_patches_local',type=str,
...
if args.wandb:
    ...
    run = wandb.init(project='voxelmorph',group= 'multimodal_96crop_t11loss_sim',
                     name= args.loss_feat+'_p8_sim01_lambda='+str(args.weight),
    config={'encoder filters':args.enc, 'decoder filters':args.dec, 'int-steps':args.int_steps,
            ...
            'similarity:':args.similarity,'weight-sp':args.weight_sp, 'layer':'0', 'patch_size':(8,8,8),
...
model, model_dir = vxm_model(model_n = 'multimodal_crop96_p8_t11loss_sim01_'+args.loss_feat+'/'+'lambda_'+str(args.weight), 
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    if epoch % 20 == 0 or (epoch+1)==args.epochs:
        ...
        with torch.no_grad():
            for i, data in enumerate(val_loader):
                    ...
                    moved_mask, fixed_mask, target, y_source, flow = run_epoch_multimodal_t11loss(data=data, model=model, 
                                                                        transform_model=transform_model, transformer = transformer,
    ...
    for i, data in enumerate(training_loader):
            ...
            source, target, y_source, flow = run_epoch_multimodal_t11loss(
                data=data, model=model, transform_model=transform_model, transformer=transformer,device=device, task='train', brainOneHotd=brainOneHotd)
